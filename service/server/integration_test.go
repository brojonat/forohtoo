...
func TestServerIntegration(t *testing.T) {
	...
	srv := server.New(":0", cfg, store, temporalClient, nil, nil, nil, nil, logger) // :0 assigns random available port, nil for helius/nats/SSE/metrics
	...
	go func() {
		...
		srv = server.New(testAddr, cfg, store, temporalClient, nil, nil, nil, nil, logger) // nil for helius/nats/SSE/metrics
		serverAddr <- testAddr
