...
func TestServerIntegration(t *testing.T) {
	...
	srv := server.New(":0", cfg, store, temporalClient, nil, nil, logger) // :0 assigns random available port, nil for SSE and metrics
	...
	go func() {
		...
		srv = server.New(testAddr, cfg, store, temporalClient, nil, nil, logger) // nil for SSE and metrics
		serverAddr <- testAddr
